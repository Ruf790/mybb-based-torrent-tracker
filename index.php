...
while ($_active_users = $db->fetch_array($_wgo_query->result))  // Используем $_wgo_query->result
{
...
$_guests = ts_nf($db->num_rows($_qsquery->result));  // Используем $_qsquery->result
...
$_most_ever = $db->num_rows($_wgo_query2->result) + $_guests;  
...
while ($_user = $db->fetch_array($_wgo_query2->result))  
{
...
while ($row = $db->fetch_array($resultPopularTorrents->result)) 
{
...
while ($row = $db->fetch_array($resultActiveTorrents->result)) 
{
...
if ($db->num_rows($Query->result) > 0) {
    ...
    while ($Torrent = $db->fetch_array($Query->result)) 
	{
