    ...
    if ($keywords) {
        ...
        switch ($search_type) {
            ...
            case 't_uploader':
                ...
                if ($db->num_rows($user_query->result) > 0) {
                    $user = $db->fetch_array($user_query->result);
                    $extraquery[] = "t.owner = ?";
...
// Используем $countquery->result для подсчета строк
$threadcount = $db->num_rows($countquery->result);
...
$queryhash = md5($sql . serialize($data_params)); 
$TotalTorrents = array();

if ($db->num_rows($Query->result))  
{
    while($t = $db->fetch_array($Query->result))  
    {
        $TotalTorrents[] = $t;
    }
}
