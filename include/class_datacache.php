...
<br />\n";
	...
	function update_moderators()
	{
		...
		$query = $db->sql_query_prepared("
           ...
           WHERE m.isgroup = ?
           ORDER BY u.username", [0]); // 0 соответствует '0' в isgroup
    ...
    $threads = [];
    $statslimit = 15; // ограничение в int, а не в строке

    // Подготавливаем безопасный запрос
    $sql = "
        SELECT tid, subject, replies, fid, uid
        FROM tsf_threads
        WHERE visible = ?
    ...
    $query = $db->sql_query_prepared($sql, [1, $statslimit]);

    while ($thread = $db->fetch_array($query)) {
        $threads[] = $thread;
    }

    // Обновляем кэш/статистику
    ...
    $threads = [];
    $statslimit = 15; // число, а не строка

    // SQL с подготовкой
    $sql = "
        SELECT tid, subject, views, fid, uid
        FROM tsf_threads
        WHERE visible = ?
        ORDER BY views DESC
        LIMIT ?
	...
	function update_statistics()
	{
		...
		//$query = $db->simple_select('users', 'id, username, referrals', 'referrals>0', array('order_by' => 'referrals', 'order_dir' => 'DESC', 'limit' => 1));
	...
	function update_news()
   {
	...
	function update_torrents()
	{
		...
		//$extra1 = ($showimages == 'yes' ? ',t.t_image,' : '');
