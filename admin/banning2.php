...
if($mybb->input['action'] == "lift")
{
	...
	if(!$user || (is_super_admin($user['uid']) && ($CURUSER['id'] != $user['uid'] && !is_super_admin($CURUSER['id']))))
	{
		flash_message('cannot_perform_action_super_admin_general', 'error');
...
if($mybb->input['action'] == "edit")
{
	...
	if($mybb->request_method == "post")
	{
		...
		if(!$errors)
		{
			...
			flash_message('success_ban_updated', 'success');
			admin_redirect("index.php?act=banning2");
		}
	}
...
'.generate_select_box('bantime', $length_list, $mybb->input['bantime'], array('id' => 'bantime')).'
	...
	<script type="text/javascript" src="../scripts/select2/select2.min222222222222.js"></script>
