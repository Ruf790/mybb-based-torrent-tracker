...
class Moderation
{
    ...
    function unapprove_threads($tids)
	{
		...
		$query = $db->simple_select("tsf_threads", "*", "tid IN ($tids_list)");
		...
		while($thread = $db->fetch_array($query))
		{
			...
			if($thread['visible'] == 1 || $thread['visible'] == -1)
			{
				...
				{
					$query2 = $db->simple_select("tsf_posts", "COUNT(pid) AS posts, uid", "tid='{$thread['tid']}' AND (visible='1' OR pid='{$thread['firstpost']}') AND uid > 0 GROUP BY uid");
		...
		$db->update_query("tsf_threads", $approve, "tid IN ($tid_list)");
		// Unapprove redirects, too
		$redirect_tids = array();
		$query = $db->simple_select('tsf_threads', 'tid', "closed IN ({$tid_moved_list})");
		...
		{
			$db->update_query("tsf_posts", $approve, "pid IN (".implode(',', $posts_to_unapprove).")");
		}
	...
	function approve_posts($pids)
	{
		...
		if(!empty($pids))
		{
			...
			$db->update_query("tsf_posts", $approve, $where);
		}
	...
	function approve_threads($tids)
	{
		...
		if(!empty($tid_list))
		{
			...
			{
				$db->update_query("tsf_posts", $approve, "pid IN (".implode(',', $posts_to_approve).")");
			}
	...
	function delete_poll($pid)
	{
		...
		$db->delete_query("tsf_polls", "pid='$pid'");
		$db->delete_query("tsf_pollvotes", "pid='$pid'");
		$pollarray = array(
			'poll' => '0',
		);
		$db->update_query("tsf_threads", $pollarray, "poll='$pid'");
	...
	function close_threads($tids)
	{
		...
		$db->update_query("tsf_threads", $openthread, "tid IN ($tid_list) AND closed NOT LIKE 'moved|%'");
	...
	function open_threads($tids)
	{
		...
		$db->update_query("tsf_threads", $closethread, "tid IN ($tid_list)");
	...
	function stick_threads($tids)
	{
		...
		$db->update_query("tsf_threads", $stickthread, "tid IN ($tid_list)");
	...
	function unstick_threads($tids)
	{
		...
		$db->update_query("tsf_threads", $unstickthread, "tid IN ($tid_list)");
	...
	function delete_thread($tid)
	{
		...
		$query = $db->simple_select('tsf_posts', 'pid, uid, visible', "tid='{$tid}'");
		$pids = array();
		...
		if(!empty($pids))
		{
			$pids = implode(',', array_map('intval', $pids)); // обязательно! безопасно приводим к числам
			...
			$db->delete_query("comment_files", "post_id IN ($pids)");
		...
		$db->delete_query("tsf_threads", "tid='$tid'");
		$query = $db->simple_select('tsf_threads', 'tid', "closed='moved|$tid'");
		while($redirect_tid = $db->fetch_field($query, 'tid'))
		{
	    ...
	    $db->delete_query("tsf_threadsread", "tid='$tid'");
	...
	function delete_post($pid)
	{
        ...
        // === Удаляем картинки, привязанные к POST ===
        $files = $db->simple_select("comment_files", "*", "post_id = " . (int)$pid);
        while ($file = $db->fetch_array($files)) 
	    {
            if (is_file($file['file_path'])) 
		    {
        ...
        // Удаляем записи из таблицы
        $db->delete_query("comment_files", "post_id = " . (int)$pid);
		...
		update_forum_counters($post['fid'], $update_array22);
	...
	function merge_posts($pids=array(), $tid=0, $sep="new_line")
	{
		...
		$query = $db->sql_query("
			SELECT p.pid, p.uid, p.fid, p.tid, p.visible, p.message, t.visible AS threadvisible, t.replies AS threadreplies, t.firstpost AS threadfirstpost, COUNT(a.aid) AS attachmentcount
			FROM tsf_posts p
			LEFT JOIN tsf_threads t ON (t.tid=p.tid)
		...
		$db->update_query("tsf_posts", $mergepost, "pid = '{$masterpid}'");

		// Delete the extra posts
		$db->delete_query("tsf_posts", "pid IN({$pidin}) AND pid != '{$masterpid}'");
		...
		$query = $db->simple_select("tsf_threads", "tid, uid, fid, visible", "firstpost IN({$pidin}) AND firstpost != '{$masterpid}'");
		while($thread = $db->fetch_array($query))
		{
			...
			$query = $db->simple_select("tsf_posts", "pid, uid, visible", "tid='{$thread['tid']}'", array('order_by' => 'dateline, pid', 'limit' => 1));
			$new_firstpost = $db->fetch_array($query);
			if($thread['visible'] != $new_firstpost['visible'])
			{
				$db->update_query("tsf_posts", array('visible' => $thread['visible']), "pid='{$new_firstpost['pid']}'");
				// Correct counters
			...
			if($new_firstpost['uid'] != $thread['uid'] && $thread['visible'] == 1)
			{
	...
	function merge_threads($mergetid, $tid, $subject)
	{
		...
		$db->update_query("tsf_posts", $sqlarray, "tid='{$mergetid}'");

		$sqlarray = array(
			"closed" => "moved|{$tid}",
		);
		$db->update_query("tsf_threads", $sqlarray, "closed='moved|{$mergetid}'");
		$sqlarray = array(
			"tid" => $tid,
		);
		...
		$query = $db->simple_select("tsf_threadsubscriptions", "tid, uid", "tid='{$mergetid}' OR tid='{$tid}'");
		...
		if(!empty($subscriptions[$mergetid]))
 		{
			...
			if(!empty($update_users))
			{
				...
				$db->update_query("tsf_threadsubscriptions", $update_array, "tid = '{$mergetid}' AND uid IN ({$update_users})");
			}
 		}

		// Remove source thread subscriptions
		$db->delete_query("tsf_threadsubscriptions", "tid = '{$mergetid}'");
		...
		// Therefore resync the visible field to make sure they're the same if they're not
		$query = $db->simple_select("tsf_posts", "pid, uid, visible", "tid='{$tid}'", array('order_by' => 'dateline, pid', 'limit' => 1));
		$new_firstpost = $db->fetch_array($query);
		if($thread['visible'] != $new_firstpost['visible'])
		{
			$db->update_query("tsf_posts", array('visible' => $thread['visible']), "pid='{$new_firstpost['pid']}'");
		...
		if($thread['uid'] != $new_firstpost['uid'] && $thread['visible'] == 1)
		{
	...
	function move_thread($tid, $new_fid, $method="redirect", $redirect_expire=0)
	{
		...
		switch($method)
		{
			case "redirect": // move (and leave redirect) thread
				...
				$query = $db->simple_select('tsf_threads', 'tid', "closed='moved|$tid' AND fid='$new_fid'");
				while($redirect_tid = $db->fetch_field($query, 'tid'))
				{
					$this->delete_thread($redirect_tid);
				}
				...
				$db->update_query("tsf_threads", $changefid, "tid='$tid'");
				$db->update_query("tsf_posts", $changefid, "tid='$tid'");
				...
				$query = $db->simple_select("tsf_threads", "tid", "closed LIKE 'moved|".(int)$tid."' AND fid='".(int)$new_fid."'");
				while($redirect_tid = $db->fetch_field($query, 'tid'))
				{
					$this->delete_thread($redirect_tid);
				}
			...
			case "copy":// copy thread
				...
				$newtid = $db->insert_query("tsf_threads", $threadarray);

				if($thread['poll'] != 0)
				{
					$query = $db->simple_select("tsf_polls", "*", "tid = '{$thread['tid']}'");
					...
					$new_pid = $db->insert_query("tsf_polls", $poll_array);

					$query = $db->simple_select("tsf_pollvotes", "*", "pid = '{$poll['pid']}'");
					while($pollvote = $db->fetch_array($query))
					{
						...
						$db->insert_query("tsf_pollvotes", $pollvote_array);
					}

					$db->update_query("tsf_threads", array('poll' => $new_pid), "tid='{$newtid}'");
				}

				$query = $db->simple_select("tsf_posts", "*", "tid = '{$thread['tid']}'");
				while($post = $db->fetch_array($query))
				{
					$post_array = array(
						'tid' => $newtid,
					...
					$pid = $db->insert_query("tsf_posts", $post_array);

					// Properly set our new firstpost in our new thread
					if($thread['firstpost'] == $post['pid'])
					{
						$db->update_query("tsf_threads", array('firstpost' => $pid), "tid='{$newtid}'");
					}
					...
					{
						$db->update_query("tsf_posts", array('message' => $db->escape_string($post['message'])), "pid='{$pid}'");
					}
				}
			...
			case "move": // plain move thread
				...
				$db->update_query("tsf_threads", $sqlarray, "tid='$tid'");
				$db->update_query("tsf_posts", $sqlarray, "tid='$tid'");
		...
		while($posters = $db->fetch_array($query))
		{
			...
			if($pcount > 0)
			{
				update_user_counters($posters['id'], array('postnum' => "+$pcount"));
			}
			elseif($pcount < 0)
			{
				update_user_counters($posters['id'], array('postnum' => $pcount));
	...
	function split_posts($pids, $tid, $moveto, $newsubject, $destination_tid=0)
	{
		...
		if($destination_tid == 0)
		{
			...
			$newtid = $db->insert_query("tsf_threads", $newthread);
		...
		$original_posts_query = $db->sql_query("
		...
		$db->update_query("tsf_posts", $sqlarray, "pid IN ($pids_list)");
		...
		while($post = $db->fetch_array($original_posts_query))
		{
			...
			if($post['firstpost'] == $post['pid'])
			{
				// In some cases the first post of a thread changes
				// Therefore resync the visible field to make sure they're the same if they're not
				$query = $db->simple_select("tsf_posts", "pid, visible, uid", "tid='{$post['tid']}'", array('order_by' => 'dateline, pid', 'limit' => 1));
				$new_firstpost = $db->fetch_array($query);
				...
				{
					$db->update_query("tsf_posts", array('visible' => $post['threadvisible']), "pid='{$new_firstpost['pid']}'");
			...
			if($post['pid'] == $post_info['pid'] && $post['dateline'] < $newthread['dateline'])
			{
				...
				{
					$db->update_query("tsf_posts", array('visible' => $newthread['visible']), "pid='{$post['pid']}'");
		...
		if(is_array($thread_counters))
		{
			foreach($thread_counters as $tid => $counters)
			{
				if($tid == $newtid)
				{
					...
					$query = $db->simple_select("tsf_posts", "pid", "tid='$newtid'", array('order_by' => 'dateline, pid', 'limit' => 1));
					$newthread = $db->fetch_array($query);
					$sqlarray = array(
						"subject" => $newsubject,
						"replyto" => 0
					);
					$db->update_query("tsf_posts", $sqlarray, "pid='{$newthread['pid']}'");
				}
				else
				{
					...
					$db->update_query("tsf_posts", $sqlarray, "pid='{$oldthread['pid']}'");
				}
