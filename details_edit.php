...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //$t_link = isset($_POST['t_link']) ? htmlspecialchars($_POST['t_link']) : '';
...
if (!empty($t_link)) {
    ...
    if (preg_match('@^https:\/\/www\.imdb\.com\/title\/(.*)\/$@isU', $t_link, $result)) {
        ...
        // Если ссылка невалидная, но поле не пустое - можно очистить поле
        $Update_tlink = array(
            "t_link" => '',
            "tags" => ''
        );
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Инициализация состояния nukereason
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Edit Torrent <?php echo htmlspecialchars($torrent['name'] ?? ''); ?></h5>
      ...
      <div class="modal-body">
        <form method="post" id="insert_form" enctype="multipart/form-data" onsubmit="return submitForm(event)">
          ...
          <!-- Category -->
          <label for="category">Select Torrent Category</label>
          ...
          <input type="text" class="form-control mt-3" name="t_link" size="70" value="<?php echo htmlspecialchars($t_link ?? ''); ?>" />
          <br />
