...
function make_searchable_forums($pid=0, $selitem=0, $addselect=1, $depth='')
{
	...
	{
		// Get Forums
		$query = $db->simple_select("tsf_forums", "pid,disporder,fid,password,name", "linkto='' AND active!=0", array('order_by' => "pid, disporder"));
...
{
	global $db, $mybb, $is_mod, $CURUSER, $usergroups;
	...
	{
		return "({$aliasdot}visible = 1 OR ({$aliasdot}visible = 0 AND {$aliasdot}uid = {$CURUSER['id']}))";
	}
	return "{$aliasdot}visible = 1";
}
...
function clean_keywords_ft($keywords)
{
	...
	$min_word_length = (int) $minsearchword;
...
function privatemessage_perform_search_mysql($search)
{
				...
				if(!$inquote)
				{
					...
					for($i=0; $i < $count_matches; ++$i)
					{
						...
						else
						{
							...
							{
								$error_minsearchlength = sprintf('error_minsearchlength', $minsearchword);
								stderr($error_minsearchlength);
							}
	...
	if($search['sender'])
	{
		...
		$query = $db->simple_select("users", "id", "{$field} LIKE '%".$db->escape_string_like($search['sender'])."%'");
...
function perform_search_mysql($search)
{
	...
	else
	{
		...
		if($firstposts)
		{
			$query = $db->simple_select("tsf_posts", "pid", "pid IN ($firstposts) {$plain_post_visiblesql} {$limitsql}");
			while($post = $db->fetch_array($query))
			{
				$posts[$post['pid']] = $post['pid'];
			}
...
function perform_search_mysql_ft($search)
{
	...
	if(!empty($search['author']))
	{
		...
		{
			$query = $db->simple_select("users", "id", "username LIKE '%".$db->escape_string_like($search['author'])."%'");
