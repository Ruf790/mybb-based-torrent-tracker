...
async function deleteBulkSequential(ids) {
    const bar = document.getElementById('bulkActionBar');
    let deleted = 0;
    let failed  = 0;
    ...
    for (let i = 0; i < ids.length; i++) {
        ...
        bar.innerHTML = '<div class="d-flex align-items-center gap-2">'
            ...
            + ' Deleting ' + (i + 1) + ' of ' + ids.length + '...</div>';

        try {
            const formData = new FormData();
            formData.append('empid', id);
            const r    = await fetch('deleteRecords.php', {
                method:  'POST',
                body:    formData,
    ...
    if (deleted > 0 && failed === 0) {
        showToast(deleted + ' users deleted successfully', 'success');
