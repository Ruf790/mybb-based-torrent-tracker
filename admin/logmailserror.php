...
/***********************************************/
  ...
  else
  {
    ...
    {
      ($db->sql_query ('DELETE FROM mailerrors WHERE mid IN (' . implode (', ', $_POST['logid']) . ')') );
      echo '
  ...
  print '<h2>Search System Error Email Log:</h2> <label><input type="text" class="form-control form-control-sm border" name="query" size="40" value="' . htmlspecialchars ($searchstr) . '"></label>';
  ...
  $res = $db->sql_query ('SELECT COUNT(*) FROM mailerrors');
...
$multipage = multipage($threadcount, $perpage, $page, $page_url);
  ...
  ($res = $db->sql_query ('' . 'SELECT eid, dateline, message, fromaddress, toaddress, error FROM mailerrors ORDER BY dateline DESC LIMIT '.$start.', ' . $perpage . ''));
