...
if($mybb->input['action'] == "dstswitch" && $mybb->request_method == "post" && $CURUSER['id'] > 0)
{
	if($CURUSER['dstcorrection'] == 2)
	{
		if($CURUSER['dst'] == 1)
		{
	...
	$db->update_query("users", $update_array, "id='{$CURUSER['id']}'");
	if(!isset($mybb->input['ajax']))
	{
		redirect("index.php", 'dst_settings_updated');
	}
	else
	{
		echo "done";
...
elseif($mybb->input['action'] == "markread")
{
	...
	if(isset($mybb->input['fid']))
	{
		...
		if(!isset($mybb->input['ajax']))
		{
			redirect(get_forum_link($mybb->input['fid']), $lang->misc['redirect_markforumread']);
		}
		else
		{
			echo 1;
...
elseif($mybb->input['action'] == "help")
{
	...
	if($hid)
	{
		$query = $db->sql_query("
			...
			FROM helpdocs h
			LEFT JOIN helpsections s ON (s.sid=h.sid)
			WHERE h.hid='{$hid}'
		");
		...
		if($helpdoc['section'] != 0 && $helpdoc['enabled'] != 0)
		{
			...
			if($helpdoc['hid'] == 3)
			{
				$helpdoc['document'] = sprintf($helpdoc['document'], $mybb->post_code);
			}
...
elseif($mybb->input['action'] == "whoposted")
{
	...
	$whoposted = '';
	$query = $db->sql_query("
		SELECT COUNT(p.pid) AS posts, p.username AS postusername, u.id, u.username, u.usergroup, u.displaygroup
		FROM tsf_posts p
		LEFT JOIN users u ON (u.id=p.uid)
		WHERE tid='".$tid."' AND $show_posts
		GROUP BY u.id, p.username, u.id, u.username, u.usergroup, u.displaygroup
		".$sortsql."
	");
	while($poster = $db->fetch_array($query))
	{
		...
		if($modal)
		{
			...
			if($poster['id'])
			{
				$onclick = "opener.location.href='".get_profile_link($poster['id'])."'; return false;";
			}
			$profile_link = build_profile_link($poster_name, $poster['id'], '_blank', $onclick);
		}
		else
		{
			$profile_link = build_profile_link($poster_name, $poster['id']);
		}
		$numposts += $poster['posts'];
		$poster['posts'] = ts_nf($poster['posts']);
		
		$whoposted .= '
		
		<div class="row g-0 border-bottom py-2">
	<div class="col">'.$profile_link.'</div>
	<div class="col">'.$poster['posts'].'</div>
</div>';
...
function makesyndicateforums($pid=0, $selitem="", $addselect=true, $depth="")
{
	...
	{
		// Get Forums
		$query = $db->simple_select("tsf_forums", "*", "linkto = '' AND active!=0", array('order_by' => 'pid, disporder'));
		...
		foreach($forumcache[$pid] as $key => $main)
		{
			foreach($main as $key => $forum)
			{
				...
				//{
					...
					if($forum['password'] == '' && !in_array($forum['fid'], $unexp) || $forum['password'] && isset($mybb->cookies['forumpass'][$forum['fid']]) && my_hash_equals($mybb->cookies['forumpass'][$forum['fid']], md5($CURUSER['id'].$forum['password'])))
					{
