...
if ($do == 1) {
    ...
    <div class="card-body p-0">
        ...
        echo '<form method="post" action="' . $_SERVER['SCRIPT_NAME'] . '">
                <input type="hidden" name="act" value="update_external_torrents">
...
if ($et = $db->fetch_array($query)) {
    ...
    echo '<script>
        ...
        function countdown() {
            x6115--;
            if (x6115 === 0) {
                location.href = "' . $_SERVER['SCRIPT_NAME'] . '?act=update_external_torrents&do=2&wait=' . $wait . '&last_id=' . $id . '&updated_ids=' . $new_updated_ids_str . '";
            } else {
