...
if ($type == 1) 
{
    ...
    if ($limit == 10 || $subtype == "ul") {
        $order = "uploaded DESC";
        ($r = $db->sql_query($mainquery . " ORDER BY " . $order . " LIMIT " . $limit)) || sqlerr(__FILE__, 233);
    ...
    if ($limit == 10 || $subtype == "dl") {
        ...
        ($r = $db->sql_query($mainquery . " ORDER BY " . $order . " LIMIT " . $limit)) || sqlerr(__FILE__, 240);
    ...
    if ($limit == 10 || $subtype == "uls") {
        $order = "upspeed DESC";
        ($r = $db->sql_query($mainquery . " ORDER BY " . $order . " LIMIT " . $limit)) || sqlerr(__FILE__, 247);
    ...
    if ($limit == 10 || $subtype == "dls") {
        ...
        ($r = $db->sql_query($mainquery . " ORDER BY " . $order . " LIMIT " . $limit)) || sqlerr(__FILE__, 254);
    ...
    if ($limit == 10 || $subtype == "bsh") {
        ...
        ($r = $db->sql_query($mainquery . $extrawhere . " ORDER BY " . $order . " LIMIT " . $limit)) || sqlerr(__FILE__, 262);
    ...
    if ($limit == 10 || $subtype == "wsh") {
        ...
        $extrawhere = " AND downloaded > 1073741824";
        ($r = $db->sql_query($mainquery . $extrawhere . " ORDER BY " . $order . " LIMIT " . $limit)) || sqlerr(__FILE__, 270);
...
$notin_ids = array_map('intval', explode(',', $notin)); // [7,6,5]
$placeholders = implode(',', array_fill(0, count($notin_ids), '?'));
...
$sql = "
    ...
    WHERE usergroup NOT IN ($placeholders)
    ORDER BY uprate DESC
    LIMIT ?
";

// параметры: сначала все id для NOT IN, потом limit
$params = array_merge($notin_ids, [(int)$limit]);
...
function usertable($res, $frame_caption) {
    ...
    while ($a = $db->fetch_array($res)) {
        ...
        // Fix DivisionByZeroError - безопасное вычисление ratio
        $ratio_value = 0;
        $ratio_display = "∞";
        $color = "#000000";
