...
?>';
  ...
  if ($action == 'edituser')
  {
	...
	//$where = array ('Cancel' => $profilelink_plain2, 'User Threads' => $BASEURL . '/search.php?action=finduserthreads&uid=' . $userdata['id'], 'User Posts' => $BASEURL . '/search.php?action=finduser&uid=' . $userdata['id'], 'Reset Passkey' => $_SERVER['SCRIPT_NAME'] . '?action=resetpasskey&userid=' . $userdata['id'], 'IP Info' => $BASEURL . '/admin/index.php?act=ip_info&userid=' . $userdata['id'], 
...
<div class="container mt-3">
    <div class="float-start">
	 ...
	 <div class="mb-3">
    <button onclick="jumpto('<?= $BASEURL ?>/<?= get_profile_link($userdata['id']) ?>');" class="btn btn-primary me-2 mb-2" type="button">
        <i class="bi bi-x-circle text-white"></i> Cancel
    </button>
    <button onclick="jumpto('<?= $BASEURL ?>/search.php?action=finduserthreads&uid=<?= $userdata['id'] ?>');" class="btn btn-primary me-2 mb-2" type="button">
        <i class="bi bi-card-text text-white"></i> User Threads
    </button>
    <button onclick="jumpto('<?= $BASEURL ?>/search.php?action=finduser&uid=<?= $userdata['id'] ?>');" class="btn btn-primary me-2 mb-2" type="button">
        <i class="bi bi-chat-left-text text-white"></i> User Posts
    </button>
    <button onclick="jumpto('<?= $BASEURL ?>/admin/index.php?act=ip_info&userid=<?= $userdata['id'] ?>');" class="btn btn-primary me-2 mb-2" type="button">
        <i class="bi bi-info-circle text-white"></i> IP Info
    </button>
    <button onclick="jumpto('<?= $_SERVER['SCRIPT_NAME'] ?>?action=resetpasskey&userid=<?= $userdata['id'] ?>');" class="btn btn-primary me-2 mb-2" type="button">
        <i class="bi bi-key text-white"></i> Reset Passkey
    </button>
    <button onclick="jumpto('<?= $_SERVER['SCRIPT_NAME'] ?>?action=deleteaccount&userid=<?= $userdata['id'] ?>');" class="btn btn-primary me-2 mb-2" type="button">
...
if ($action == 'updateuser')
{
  ...
  <div class="alert alert-danger">
   ...
   if (isset($_POST['avatar'])) 
   {
    ...
    redirect("edituser.php?action=edituser&userid={$userdata['id']}", "The account ({$userdata['id']}) has been updated...");
    return 1;
}
  ...
  if ($action == 'banaccount')
  {
    ...
    redirect ('edituser.php?action=edituser&userid=' . $userid, 'The account (' . $userid . ') has been updated...');
    return 1;
  }
...
stdfoot();
  ...
  if ($action == 'resetpasskey')
  {
    ...
    if (isset($_POST['sure']) && $_POST['sure'] == 1)
    {
        ...
        redirect('edituser.php?action=edituser&userid=' . $userdata['id'], 'The passkey has been updated...');
        return 1;
    }
