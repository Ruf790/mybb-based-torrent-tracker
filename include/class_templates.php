...
class templates
{
    ...
    function cache($templates)
    {
        ...
        // Создаем плейсхолдеры для каждого шаблона
        $placeholders = implode(',', array_fill(0, count($names), '?'));

        $theme['templateset'] = 2;

        // Собираем параметры для prepared statement
        $params = $names;
        $params[] = -2;                  // sid IN (-2, -1, templateset)
        $params[] = -1;
        $params[] = (int)$theme['templateset'];

        // Создаем плейсхолдеры для sid
        $sid_placeholders = implode(',', array_fill(0, 3, '?'));

        $sql = "SELECT title, template 
            FROM templates
...
else
{
    $query = $db->sql_query_prepared("
        ...
        ORDER BY sid DESC 
        LIMIT 1
    ", [$db->escape_string($title), (int)$theme['templateset']]);
}
