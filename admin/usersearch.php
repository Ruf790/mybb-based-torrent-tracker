...
echo '<script src="https://cdn.jsdelivr.net/npm/flatpickr/dist/l10n/ru.js"></script>';
...
if (!empty($params)) {
    // Если есть параметры, используем подготовленный запрос
    $count_res = $db->sql_query_prepared($sql_count, $params);
} else {
    // Если параметров нет, используем обычный запрос
    $count_res = $db->sql_query($sql_count);
}

$count_arr = $db->fetch_array($count_res);
...
$params_for_data[] = $start;
$params_for_data[] = $perpage;

if (!empty($params_for_data)) {
    // Используем подготовленный запрос с параметрами
    $query = $db->sql_query_prepared($sql_data, $params_for_data);
} else {
    // Если нет параметров (маловероятно для этого запроса)
    $query = $db->sql_query($sql_data);
}

$num = $db->num_rows($query->result);
    ...
    if ($num > 0) {
        ...
        while ($u = $db->fetch_array($query->result))
		{
