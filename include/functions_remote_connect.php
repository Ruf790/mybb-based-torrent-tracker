...
function fetch_remote_file(string $url, array|false $post_data = [], int $max_redirects = 20): string|false
{
    ...
    if (!$addresses) {
        error_log('fetch_remote_file Error: Cannot resolve host — ' . $url_components['host']);
        return false;
    }
    ...
    if (!empty($disallowed_remote_addresses)) {
        ...
        if ($packed_address === false) {
            error_log('fetch_remote_file Error: Invalid IP address — ' . $destination_address);
            return false;
        }
    ...
    if (function_exists('curl_init')) {
        return fetch_via_curl($url, $url_components, $destination_address, $post_body, $post_data, $max_redirects);
    }

    // fsockopen fallback
    if (function_exists('fsockopen')) {
        return fetch_via_socket($url, $url_components, $destination_address, $post_body, $post_data, $max_redirects);
    }

    return false;
}

function fetch_via_curl(string $url, array $url_components, string $destination_address, string $post_body, array $post_data, int $max_redirects): string|false
{
    ...
    if ($fetch_header && is_string($response)) {
        ...
        if (in_array(curl_getinfo($ch, CURLINFO_HTTP_CODE), [301, 302], true)) {
            preg_match('/^Location:(.*?)(?:\n|$)/im', $header, $matches);
            if ($matches) {
                $data = fetch_remote_file(trim(array_pop($matches)), $post_data, $max_redirects - 1);
            }
        } else {
            $data = $body;
        }
...
function fetch_via_socket(string $url, array $url_components, string $destination_address, string $post_body, array $post_data, int $max_redirects): string|false
{
    $url_components['path']  ??= '/';
    ...
    if ($max_redirects > 0 && (str_contains($status_line, ' 301 ') || str_contains($status_line, ' 302 '))) {
        preg_match('/^Location:(.*?)(?:\n|$)/im', $header, $matches);
        if ($matches) {
            return fetch_remote_file(trim(array_pop($matches)), $post_data, $max_redirects - 1);
        }
    }
