...
function build_forumbits($pid=0, $depth=1)
{
	...
	foreach($fcache[$pid] as $parent)
	{
		foreach($parent as $forum)
		{
			...
			if($permissions['canview'] != 1 && $hideprivateforums == 1)
			{
				continue;
			}
			...
			if(isset($permissions['canonlyviewownthreads']) && $permissions['canonlyviewownthreads'] == 1)
			{
				...
				if(!is_array($private_forums))
				{
					...
					{
						$fids = implode(',', $fids);
						$query = $db->simple_select("tsf_threads", "tid, fid, subject, lastpost, lastposter, lastposteruid", "uid = '{$CURUSER['id']}' AND fid IN ({$fids}) AND visible != '-2'", array("order_by" => "lastpost", "order_dir" => "desc"));
			...
			else
			{
				...
				{
					$forum['lastposter'] = 'guest'; // htmlspecialchars_uni'd when formatted later
				}
			...
			else if($depth == 3)
			{
				...
				if($donecount < $subforumsindex)
				{
					...
					if($subforumsstatusicons == 1)
					{
						...
						$statusicon = '<div class="subforumicon subforum_'.$lightbulb['folder'].' ajax_mark_read ficons_'.$forum['fid'].'" title="'.$lightbulb['altonoff'].'" id="mark_read_'.$forum['fid'].'"><i class="fa-solid fa-circle"></i></div>';
					}
				...
				if($donecount == $subforumsindex)
				{
					...
					{
						$forum_list .= $comma.sprintf($lang->global['more_subforums'], (subforums_count($fcache[$pid]) - $donecount));
					}
				}
				continue;
			}
			...
			if($forum['linkto'] == '')
			{
				...
				if($forum['viewers'] > 0)
				{
					if($forum['viewers'] == 1)
					{
						$forum_viewers_text = 'viewing_one';
					}
					else
					{
						$forum_viewers_text = sprintf('viewing_multiple', $forum['viewers']);
					}
					$forum_viewers_text_plain = $forum_viewers_text;
					
					$forum_viewers_text = '<span class="smalltext text-muted" style="font-size: 12px">'.$forum_viewers_text.'</span>';
				}
			}
...
function get_forum_lightbulb($forum, $lastpost, $locked=0)
{
	...
	else
	{
		...
		else
		{
			...
			else if($CURUSER['id'] && $threadreadcut > 0 && $threadcut > $lastpost['lastpost'])
			{
		...
		if($lastpost['lastpost'] > $forum_read && $lastpost['lastpost'] != 0)
		{
			$unread_forums++;
			$folder = "on";
			$altonoff = $lang->global['new_posts'];
		}
		// Otherwise, no new posts
		else
		{
			$folder = "off";
			$altonoff = $lang->global['no_new_posts'];
		}
	}
...
{
	global $lang, $templates, $usergroups;
	...
	if($is_mod)
	{
		...
		if($forum['unapprovedposts'])
		{
			...
			{
				$unapproved_posts_count = sprintf($lang->index['forum_unapproved_posts_count'], $forum['unapprovedposts']);
			}
			else
			{
				$unapproved_posts_count = sprintf($lang->index['forum_unapproved_post_count'], 1);
			}

			$forum['unapprovedposts'] = ts_nf($forum['unapprovedposts']);
		...
		if($forum['unapprovedthreads'])
		{
			...
			{
				$unapproved_threads_count = sprintf($lang->index['forum_unapproved_threads_count'], $forum['unapprovedthreads']);
			}
			else
			{
				$unapproved_threads_count = sprintf($lang->index['forum_unapproved_thread_count'], 1);
			}

			$forum['unapprovedthreads'] = ts_nf($forum['unapprovedthreads']);
