...
function getExtendedStats(int $torrent_id): array
{
    ...
    $stats['avg_ratio']      = $n > 0 ? number_format($totalRatio / $n, 2) : '0.00';
...
<div class="modal fade" id="userDetailsModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body" id="userDetailsContent">Loading...</div>
        </div>
    </div>
</div>
                    ...
                    <form method="GET" class="row g-2">
                        ...
                        <div class="col-md-2">
                            <select name="filter_seeder" class="form-select form-select-sm" onchange="this.form.submit()">
                                ...
                                <option value="yes" <?= ($activeFilters['filter_seeder'] ?? '') === 'yes' ? 'selected' : '' ?>>Seeders Only</option>
                                <option value="no"  <?= ($activeFilters['filter_seeder'] ?? '') === 'no'  ? 'selected' : '' ?>>Non-Seeders</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            <select name="filter_connectable" class="form-select form-select-sm" onchange="this.form.submit()">
                                ...
                                <option value="yes" <?= ($activeFilters['filter_connectable'] ?? '') === 'yes' ? 'selected' : '' ?>>Connectable</option>
                                <option value="no"  <?= ($activeFilters['filter_connectable'] ?? '') === 'no'  ? 'selected' : '' ?>>Not Connectable</option>
                            </select>
                        </div>
                        <div class="col-md-2">
            ...
            <div class="row mt-3">
                <div class="col-md-12">
                    <div class="d-flex align-items-center gap-3 p-2 bg-light rounded">
                        ...
                        <span class="small text-muted">Selected: <strong id="selectedCount">0</strong></span>
...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <div class="small text-muted">
                <i class="fas fa-hdd me-1"></i>Size: <?= mksize($torrentInfo['size']) ?> |
                <i class="fas fa-tag me-1"></i>Category: <?= htmlspecialchars($torrentInfo['category_name'] ?? 'Unknown') ?>
            </div>
        </div>
        <div class="text-end">
            ...
            <div class="row g-3 h-100">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body py-2">
                            <div class="row text-center">
                                <?php
                                $quickStats = [
                                    [$extStats['unique_users'],              'Unique Users', 'success'],
                                    [$extStats['avg_ratio'],                 'Avg Ratio',    'primary'],
                                    [mksize($extStats['total_speed']) . '/s','Total Speed',  'info'],
                                    [$extStats['completion_rate'] . '%',     'Completion',   'warning'],
                                ];
                ...
                <div class="col-12">
                    <div class="card border-warning">
                        <div class="card-header">
                            <h6 class="mb-0">
                                <i class="fas fa-exclamation-triangle me-2 text-warning"></i>H&amp;R Status
                            </h6>
                        </div>
                        <div class="card-body py-2">
                            <div class="row text-center">
                                ...
                                <div class="col-4">
                                    <a href="?id=<?= $id ?>&filter_warned=warned" class="text-decoration-none">
                                        ...
                                        <small class="text-muted">Warned</small>
                                    </a>
                                </div>
                                <div class="col-4">
    ...
    <div class="row mb-4">
        <?php
        $statCards = [
            [$stats['current_seeders'],          'Current Seeders',  'success',
             number_format(($stats['current_seeders'] / max($count, 1)) * 100, 1) . '% of snatchers'],
            [mksize($stats['total_uploaded']),   'Total Uploaded',   'primary', mksize($stats['total_uploaded'])],
            [mksize($stats['total_downloaded']), 'Total Downloaded', 'info',    mksize($stats['total_downloaded'])],
            [mkprettytime($extStats['total_seedtime']), 'Total Seed Time', 'warning',
             number_format($extStats['users_over_24h']) . ' users > 24h'],
        ];
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row text-center">
                        <?php
                        $seedDist = [
                            [$extStats['seedtime_distribution']['poor'],      '< 1 hour',   'danger'],
                            [$extStats['seedtime_distribution']['fair'],      '1-6 hours',  'warning'],
                            [$extStats['seedtime_distribution']['good'],      '6-24 hours', 'info'],
                            [$extStats['seedtime_distribution']['excellent'], '> 24 hours', 'success'],
                        ];
                        ...
                        <div class="col-md-3">
                            <div class="text-<?= $color ?>">
                                <h4><?= $val ?></h4>
                                <small class="text-muted"><?= $label ?></small>
                                <div class="small text-muted"><?= number_format($val / $uniq * 100, 1) ?>%</div>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
    ...
    $colHeaders = [
        ['username',    'fa-user',           'User'],
        ['uploaded',    'fa-upload',         'Uploaded'],
        ['downloaded',  'fa-download',       'Downloaded'],
        ['ratio',       'fa-percentage',     'Ratio'],
        ['completedat', 'fa-flag-checkered', 'Finished'],
        ['last_action', 'fa-clock',          'Last Action'],
        ['seeder',      'fa-seedling',       'Status'],
        ['seedtime',    'fa-clock',          'Seed Time'],
        ['leechtime',   'fa-hourglass-half', 'Leech Time'],
        ['warned',      'fa-exclamation-triangle', 'H&amp;R'],
    ];
    ?>
            ...
            <table class="table table-hover align-middle mb-0" id="snatchTable">
                ...
                if ($db->num_rows($res) > 0):
                    while ($row = $db->fetch_array($res)):
                        ...
                        if ($warnedVal >= 3) {
                            $hnrBadge   = "<span class='badge bg-danger'><i class='fas fa-ban me-1'></i>Banned</span>";
                            $rowClass  .= ' hnr-banned';
                        } elseif ($warnedVal >= 1) {
                            $hnrBadge   = "<span class='badge bg-warning text-dark'>"
                                        . "<i class='fas fa-exclamation-triangle me-1'></i>Warned {$warnedVal}x</span>";
                            $rowClass  .= ' hnr-warned';
                        } else {
                            $hnrBadge   = "<span class='badge bg-success'><i class='fas fa-check me-1'></i>Clean</span>";
                        }
                ?>
                <tr class="<?= trim($rowClass) ?>">
                    ...
                    <td><span class="fw-bold <?= $ratioClass ?>"><?= number_format($ratio, 2) ?></span></td>
                    <td>
                    ...
                    <td class="text-center">
                        ...
                        <a href="<?= $_SERVER['SCRIPT_NAME'] ?>?id=<?= $id ?>&amp;delete=1&amp;userid=<?= (int)$row['id'] ?>"
                           class="btn btn-sm btn-outline-danger"
                           onclick="return confirm('Delete this snatch record?')"
                           title="Delete">
