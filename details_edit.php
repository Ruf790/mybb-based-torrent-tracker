...
if (!empty($t_link)) {
    ...
    if ($res) 
	{
		...
		write_log( sprintf($lang->upload['editedtorrent'], '[URL='.$BASEURL."/".get_torrent_link($id).']<font color=red>' . $name . '</font>[/URL]', '[URL='.$BASEURL . '/'.get_profile_link($CURUSER['id']).']' . format_name($CURUSER['username'],$CURUSER['usergroup']) . '[/URL]'));
		$cache->update_torrents();
