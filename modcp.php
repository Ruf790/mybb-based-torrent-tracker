...
if($mybb->input['action'] == "do_banuser" && $mybb->request_method == "post")
{
	...
	if($mybb->input['uid'])
	{
		...
		$user = $db->fetch_array($query->result);
...
if($mybb->input['action'] == "banuser")
{
	...
	if($mybb->input['uid'])
	{
		...
		$banned = $db->fetch_array($query->result);
...
if($mybb->input['action'] == "banning")
{
	...
	$bannedusers = '';
	while($banned = $db->fetch_array($query->result))
	{
		...
		else
		{
			...
			//$timeremaining = mkprettytime($remaining, array('short' => 1, 'seconds' => false))."";
