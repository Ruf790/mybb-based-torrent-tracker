...
if(($mybb->input['action'] == "editdraft" || $mybb->input['action'] == "do_newreply") && $mybb->get_input('pid', MyBB::INPUT_INT))
{
	...
	else if($CURUSER['id'] != $post['uid'])
	{
		error('error_post_noperms');
	}
	$pid = (int)$post['pid'];
	$tid = (int)$post['tid'];
...
if(isset($forumpermissions['canonlyreplyownthreads']) && $forumpermissions['canonlyreplyownthreads'] == 1 && $thread['uid'] != $CURUSER['id'])
{
	print_no_permission();
}
...
<script>
  document.getElementById("insertBtn").addEventListener("click", function(event) {
    ...
    textarea.value += textToInsert;  // Добавляем текст в textarea
			...
			<input type="submit" class="btn btn-page" name="rem_'.$attachment['aid'].'" value="Remove" onclick="return Post.removeAttachment('.$attachment['aid'].');" />
...
{
	$daycut = TIMENOW-60*60*24;
	$query = $db->simple_select("tsf_posts", "COUNT(*) AS posts_today", "uid='{$CURUSER['id']}' AND visible !='-1' AND dateline>{$daycut}");
...
if($mybb->input['action'] == "do_newreply" && $mybb->request_method == "post")
{
	...
	{
		$query = $db->simple_select("tsf_posts p", "p.pid, p.visible", "{$user_check} AND p.tid='{$thread['tid']}' AND p.subject='".$db->escape_string($mybb->get_input('subject'))."' AND p.message='".$db->escape_string($mybb->get_input('message'))."' AND p.visible > -1 AND p.dateline>".(TIMENOW-600));
	...
	if($mybb->get_input('savedraft') && $CURUSER['id'])
	{
		$post['savedraft'] = 1;
	}
	else
	...
	else
	{
		...
		if(isset($mybb->input['quoted_ids']) && isset($mybb->cookies['multiquote']) && $multiquote != 0)
		{
		...
		if($mybb->get_input('ajax', MyBB::INPUT_INT))
		{
			...
			if($visible == 1)
			{
				...
				{
					$query = $db->simple_select("tsf_posts", "pid", "tid = '{$tid}' AND pid != '{$pid}'", array("order_by" => "pid", "order_dir" => "desc"));
				...
				if($f_postsperpage > 0)
				{
					$post_page = ceil(($postcounter + 1) / $f_postsperpage);
				}
				else
				{
					$post_page = 1;
				...
				if(($postcounter - $f_postsperpage) % 2 != 0)
				{
					$altbg = "trow1";
				}
				else
			...
			{
				redirect(get_thread_link($tid, 0, "lastpost"), $lang->newreply['redirect_newreply_moderation'], "", true);
				exit;
			}
		}
		else
...
if($mybb->input['action'] == "newreply" || $mybb->input['action'] == "editdraft")
{
	...
	if(empty($mybb->input['previewpost']) && !$reply_errors && $mybb->input['action'] != "editdraft" && !$mybb->get_input('attachmentaid', MyBB::INPUT_INT) && !$mybb->get_input('newattachment') && !$mybb->get_input('updateattachment'))
	{
		...
		if(isset($mybb->cookies['multiquote']) && $multiquote != 0)
		{
		...
		if(count($quoted_posts) > 0)
		{
			...
			if(!empty($onlyusfids))
			{
				$onlyusforums = "AND ((t.fid IN(".implode(',', $onlyusfids).") AND t.uid='{$CURUSER['id']}') OR t.fid NOT IN(".implode(',', $onlyusfids)."))";
			}
			...
			if($external_quotes > 0)
			{
				...
				{
					$multiquote_text = 'multiquote_external_one';
					$multiquote_deselect = 'multiquote_external_one_deselect';
					$multiquote_quote = 'multiquote_external_one_quote';
				}
				else
				{
					$multiquote_text = sprintf('multiquote_external', $external_quotes);
					$multiquote_deselect = 'multiquote_external_deselect';
					$multiquote_quote = 'multiquote_external_quote';
				}
...
';
		...
		{
			$friendlyquota = $lang->newreply['unlimited'];
		}
		else
		{
	...
	if($threadreview != 0)
	{
		...
		//}
		$query = $db->simple_select("tsf_posts", "COUNT(pid) AS post_count", "tid='{$tid}' AND {$visibility}");
		...
		if($numposts > $f_postsperpage)
		{
			...
			$reviewmore = '<tr>
           <td class="thead" align="center"><span class="smalltext"><strong>'.$thread_review_more.'</strong></span></td</tr>';
		   
		   
		}

		$pidin = array();
		$query = $db->simple_select("tsf_posts", "pid", "tid='{$tid}' AND {$visibility}", array("order_by" => "dateline DESC, pid DESC", "limit" => $f_postsperpage));
		...
		if(!empty($pidin))
		{
			...
			while($post = $db->fetch_array($query))
			{
				...
				<tr>
<td class="tcat"><span class="smalltext"><strong>{$lang->posted_by} '.$post['username'].' - '.$reviewpostdate.'</strong></span></td>
</tr>
<tr>
<td class="'.$altbg.' scaleimages">
'.$reviewmessage.'
</td>
</tr>
...
<tr>
<td class="thead" style="text-align: center;"><strong>{$lang->thread_review}</strong></td>
</tr>
'.$reviewbits.'
'.$reviewmore.'
</table>
	...
	if($is_mod)
	{
		...
		//{
		     $closeoption = '<input type="checkbox" class="form-check-input" name="modoptions[closethread]" value="1"'.$closecheck.' />&nbsp;<strong>Close Thread</strong>: prevent further posting in this thread';
		//}
		...
		$stickoption = '<br /><input type="checkbox" class="form-check-input" name="modoptions[stickthread]" value="1" '.$stickycheck.' />&nbsp;<strong>Stick Thread:</strong> stick this thread to the top of the forum';
	...
	if($forum['rulestype'] >= 2 && $forum['rules'])
	{
		if(!$forum['rulestitle'])
		{
			$forum['rulestitle'] = sprintf('forum_rules', $forum['name']);
		}
