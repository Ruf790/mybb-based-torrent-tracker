...
/***********************************************/
  ...
  function check_pincode ($area = 1, $maxfails = 5)
  {
    ...
    {
      $db->sql_query ('UPDATE users set enabled = \'no\', usergroup = \'' . UC_BANNED . '\' WHERE id = ' . sqlesc ($CURUSER['id']));
      stderr ('Error', 'Access Denied! You have been exceed your max pincode attempts therefore we have been disabled your account.');
      exit ();
    }

    $query = $db->sql_query ('SELECT sechash, pincode FROM pincode WHERE area = 1') OR sqlerr (__FILE__, 36);
