...
CREATE TABLE IF NOT EXISTS `{$p}invites` (
  ...
  `time_invited` int unsigned NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4;
...
SQL;
    ...
    $existing = mysqli_query($conn, "SELECT id FROM `{$p}users` WHERE `username`='{$uname}' OR `email`='{$uemail}' LIMIT 1");
    if (mysqli_num_rows($existing) > 0) {
        $row = mysqli_fetch_assoc($existing);
        $admin_id = (int)$row['id'];
        mysqli_query($conn, "UPDATE `{$p}users` SET `usergroup`=5,`password`='{$pass_hash}',`salt`='{$salt}',`ustatus`='confirmed',`enabled`='yes' WHERE `id`={$admin_id}");
    } else {
