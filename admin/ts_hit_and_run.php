...
function generate_pagination($base_url, $current_page, $total_pages, $per_page, $total_count) {
    ...
    // Очищаем URL от двойных амперсандов
...
// Генерируем пагинацию - очищаем от лишних амперсандов
...
WHERE s.finished=\'yes\' AND s.seeder=\'no\' AND (u.enabled=\'yes\' AND u.usergroup NOT IN (' . $skip_usergroups . ') AND u.ustatus=\'confirmed\') AND t.visible=\'yes\' AND (t.seeders > 0 OR t.leechers > 0) AND s.uploaded/s.downloaded < ' . $config['ts_hit_and_run']['min_share_ratio'] . $extraquery . $extraquery2 . ' ORDER by u.timeswarned DESC ' . $limit) or sqlerr(__FILE__, 157);
