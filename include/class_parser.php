...
class postParser
{
	...
	function cache_smilies()
	{
		...
		//require_once $BASEURL . '/cache/smilies.php';
	...
	function mycode_parse_post_quotes($message, $username, $text_only=false)
	{
		...
		if(isset($match[1]) && (int)$match[1])
		{
			...
			if(defined("IN_ARCHIVE"))	
			{
				...
				$linkback = '<a href="'.$url2.'" class="quick_jump"></a>';
			}
			else
			{
				$url = $BASEURL."/".get_post_link($pid)."#pid$pid";
				$linkback = '<a href="'.$url.'" class="quick_jump"></a>';
			}
		...
		else
		{
			...
			$mycode_quote = '<blockquote class="mycode_quote"><cite>'.$span.''.$username.' Wrote:'.$linkback.'</cite>'.$message.'</blockquote><br />';
			return $mycode_quote;
		}
	}
	...
	*/
	...
	function mycode_parse_img($url, $dimensions=array(), $align='')
	{
		...
		//$mycode_img = '<img src='.$url.' loading="lazy" {$width}{$height} alt="{$alt}"{$css_align} class="mycode_img" />';
	...
	*/
	function mycode_parse_video($video, $url)
	{
		...
		//$video_code = '<iframe src="https://www.facebook.com/video/embed?video_id='.$id.'" width="625" height="350" frameborder="0"></iframe>';
