...
if($mybb->input['action'] == "editdraft" || ($mybb->get_input('savedraft') && $mybb->input['tid']) || ($mybb->input['tid'] && $mybb->input['pid']))
{
	...
	$query = $db->simple_select("tsf_posts", "*", "tid='".$mybb->input['tid']."' AND visible='-2'", array('order_by' => 'dateline, pid', 'limit' => 1));
...
if($mybb->input['action'] == "do_newthread" && $mybb->request_method == "post")
{
	...
	else
	{
		...
		else if($mybb->get_input('postpoll', MyBB::INPUT_INT))
		{
		...
		{
			$redirect_newthread .= '<br />You do not have permission to view threads in this forum. You will now be returned to the forum';
			$url = get_forum_link($fid);
...
if($mybb->input['action'] == "newthread" || $mybb->input['action'] == "editdraft")
{
		...
		if(count($quoted_posts) > 0)
		{
			...
			else
			{
				...
				if($external_quotes > 0)
				{
					...
					<div id="multiquote_unloaded"><span class="smalltext">'.$multiquote_text.' <a href="./newthread.php?fid='.$fid.'&amp;load_all_quotes=1" onclick="return Post.loadMultiQuotedAll();">'.$multiquote_quote.'</a>, <a href="javascript:void(0)" onclick="Post.clearMultiQuoted(); return false;">'.$multiquote_deselect.'</a></span></div>
	...
	else if($mybb->input['action'] == "editdraft" && $CURUSER['id'])
	{
	...
	if(!empty($signature) || !empty($disablesmilies))
	{
		...
		<a class="links" data-bs-toggle="collapse" aria-expanded="false" aria-controls="collapse-1" href="#collapse-postop" role="button"><i class="fa-solid fa-gear"></i> &nbsp;Post Options:</a>
	...
	if($is_mod)
	{
		...
		//{
			 ...
			 <input type="checkbox" class="form-check-input" name="modoptions[closethread]" value="1"'.$closecheck.' />&nbsp;<b>Close Thread</b>: prevent further posting in this thread
		...
		//{
			...
			<br /><input type="checkbox" class="form-check-input" name="modoptions[stickthread]" value="1"'.$stickycheck.' />&nbsp;<b>Stick Thread:</b> stick this thread to the top of the forum
		...
		if(!empty($closeoption) || !empty($stickoption))
		{
			...
			&nbsp;&nbsp;<a class="links" data-bs-toggle="collapse" aria-expanded="false" aria-controls="collapse-1" href="#collapse-modop" role="button"><i class="fa-solid fa-screwdriver-wrench"></i> &nbsp;Moderator Options:</a>&nbsp;&nbsp;
  ...
  document.getElementById("insertBtn").addEventListener("click", function(event) {
    ...
    textarea.value += textToInsert;  // Добавляем текст в textarea
		...
		if($usage['ausage'] !== NULL)
		{
			...
			$lang->attach_usage = sprintf('attach_usage', $friendlyusage);
			
			$link_viewattachments = '<a href="usercp.php?action=attachments">{$lang->view_attachments}</a>';
		}
		else
		{
			$lang->attach_usage = "";
		...
		//{
			  ...
			  <button type="submit" class="btn btn-primary" name="newattachment" value="Add Attachment" tabindex="13"><i class="fa-solid fa-upload"></i> &nbsp;Add Attachment</button>
		...
		if($attachcount > 0)
		{
			...
			<button type="submit" class="btn btn-primary" name="updateattachment" value="Update Attachment" tabindex="12"><i class="fa-solid fa-check"></i> &nbsp;Update Attachment&nbsp;</button>
