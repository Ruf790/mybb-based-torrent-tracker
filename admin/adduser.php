  ...
  if (strtoupper ($_SERVER['REQUEST_METHOD'] == 'POST'))
  {
    ...
    if (count ($error) == 0)
    {
      ...
      if ($db->affected_rows ())
      {
        ...
        if (!empty($_POST['passhint']) && !empty($_POST['hintanswer'])) 
		{
             $user['passhint'] = intval($_POST['passhint']);
             $user['hintanswer'] = trim($_POST['hintanswer']);
...
$options = '<select name="passhint" id="passhint" class="form-select form-select-sm w-auto">';
foreach ($questions as $v => $q) {
    $selected = (isset($passhint) && $passhint == $v) ? ' selected' : '';
    $options .= "<option value=\"{$v}\"{$selected}>{$q}</option>";
}
$options .= '</select>';
...
<div class="py-3 border-bottom">
    <div class="row g-3">
        <div class="col-lg-6">
            <label for="passhint" class="form-label">' . $lang->member["sq"] . '</label>
            ' . $options . '
        </div>
        <div class="col-lg-6">
            <label for="hintanswer" class="form-label">' . $lang->member["sqa"] . '</label>
            <input type="text" class="form-control form-control-sm" name="hintanswer" id="hintanswer" value="' . (isset($hintanswer) ? htmlspecialchars_uni($hintanswer) : '') . '" autocomplete="off" />
        </div>
    </div>
</div>
